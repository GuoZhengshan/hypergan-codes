...
class Experiment:
    ...
    def train_and_eval(self):
        ...
        model = MDistMult(d, self.ent_vec_dim, self.rel_vec_dim, device, **self.kwargs)
        model = model.to(device)
        self.GAN = HyperGAN(train_data_idxs, args, model, device, len(d.entities), len(d.relations),
                            args.hidden_dim, args.hard)
        ...
        for it in range(0, self.num_iterations):
            if it % args.classify_steps == 0:
                print("Train HyperGAN")
                self.GAN.train_HyperGAN(self.GAN, args.epochs)
                self.GAN.test_ave_score(self.GAN)
            ...
            for miss_ent_domain in range(1, ary+1):
                ...
                for j in range(0, len(er_vocab_pairs), self.batch_size):
                    ...
                    if ary == 3:
                        ...
                        e_idx = [e1_idx, e2_idx, e3_idx]
                    pred = model.forward(r_idx, e_idx, miss_ent_domain)
                    pred = self.GAN.merge_confidence_weight(data_batch, label, miss_ent_domain, pred)
                    pred = pred.to(device)
            ...
            model.eval()
            with torch.no_grad():
                v_mrr, v_hit10, v_hit3, v_hit1 = self.evaluate(model, d.valid_data)
                print('Epoch %d valid, MRR=%.8f, Hits@10=%f, Hits@3=%f, Hits@1=%f' % (it, v_mrr, v_hit10, v_hit3, v_hit1))
                t_mrr, t_hit10, t_hit3, t_hit1 = self.evaluate(model, d.test_data)
                ...
                if (it-best_valid_iter) >= 50 or it + 1 == self.num_iterations:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classify_steps', default=20, type=int, help='regiven weight log every xx steps')
    parser.add_argument('--hidden_dim', default=11, type=int)
    parser.add_argument('--epochs', default=38, type=int)
