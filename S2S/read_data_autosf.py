...
def four_ary_heads(train_data, valid_data, test_data):
    all_r = train_data[:,0].tolist() + valid_data[:,0].tolist() + test_data[:,0].tolist()
    all_e1 = train_data[:,1].tolist() + valid_data[:,1].tolist() + test_data[:,1].tolist()
    all_e2 = train_data[:,2].tolist() + valid_data[:,2].tolist() + test_data[:,2].tolist()
    all_e3 = train_data[:,3].tolist() + valid_data[:,3].tolist() + test_data[:,3].tolist()
    all_e4 = train_data[:,4].tolist() + valid_data[:,4].tolist() + test_data[:,4].tolist()
    
    n_ent = len(set(all_e1+all_e2+all_e3+all_e4))
    
    e1s, e2s, e3s, e4s = defaultdict(lambda: set()), defaultdict(lambda: set()), defaultdict(lambda: set()), defaultdict(lambda: set())
    
    for r, e1, e2, e3, e4 in zip(all_r, all_e1, all_e2, all_e3, all_e4):
        e1s[(r, e2, e3, e4)].add(e1)
        e2s[(r, e1, e3, e4)].add(e2)
        e3s[(r, e1, e2, e4)].add(e3)
        e4s[(r, e1, e2, e3)].add(e4)
        
    e1_sp, e2_sp, e3_sp, e4_sp = {}, {}, {}, {}
    for k in e1s.keys():
        e1_sp[k] = torch.sparse.FloatTensor(torch.LongTensor([list(e1s[k])]),
                                                   torch.ones(len(e1s[k])), torch.Size([n_ent]))
    ...
    for k in e4s.keys():
        e4_sp[k] = torch.sparse.FloatTensor(torch.LongTensor([list(e4s[k])]),
                                                   torch.ones(len(e4s[k])), torch.Size([n_ent]))
        
    #print("ent size:", len(e1s), len(e2s), len(e3s))
        
    return e1_sp, e2_sp, e3_sp, e4_sp
