        ...
        for epoch in range(n_epoch):
            ...
            for facts in batch_by_size(n_batch, train_data, n_sample=n_train):
                ...
                if self.n_arity == 3:
                    ...
                    loss = self.model.forward_quad(facts, arch)                
                    loss.backward()
...
#            e3_embed = self.model.ent_embed(e3).view(-1, self.K, length)
    ...
    def quad_evaluate(self, test_data, e1_sp, e2_sp, e3_sp, e4_sp, arch = None):
        ...
        for facts in batch_by_size(self.args.test_batch_size, test_data):
            ...
            if self.GPU:
                r, e1, e2, e3, e4 = facts[:,0].cuda(), facts[:,1].cuda(), facts[:,2].cuda(), facts[:,3].cuda(), facts[:,4].cuda()
            else:
                r, e1, e2, e3, e4 = facts[:,0], facts[:,1], facts[:,2], facts[:,3], facts[:,4]
    ...
    #      e3_embed = self.model.ent_embed(e3).view(-1, self.K, length)
            ...
            e2_embed = self.model.input_dropout(self.model.bne(self.model.ent_embed(e2))).view(-1, self.K, length)
            e3_embed = self.model.input_dropout(self.model.bne(self.model.ent_embed(e3))).view(-1, self.K, length)
            e4_embed = self.model.input_dropout(self.model.bne(self.model.ent_embed(e4))).view(-1, self.K, length)
            
            e1_scores = F.softmax(self.model.quad_neg_other(r_embed, e2_embed, e3_embed, e4_embed, max_idx, 1), dim=1).data
            e2_scores = F.softmax(self.model.quad_neg_other(r_embed, e1_embed, e3_embed, e4_embed, max_idx, 2), dim=1).data
            e3_scores = F.softmax(self.model.quad_neg_other(r_embed, e1_embed, e2_embed, e4_embed, max_idx, 3), dim=1).data
            e4_scores = F.softmax(self.model.quad_neg_other(r_embed, e1_embed, e2_embed, e3_embed, max_idx, 4), dim=1).data
        
            for idx in range(len(r)):        
                r_idx, e1_idx, e2_idx, e3_idx, e4_idx = int(r[idx].data.cpu().numpy()), int(e1[idx].data.cpu().numpy()), int(e2[idx].data.cpu().numpy()), int(e3[idx].data.cpu().numpy()), int(e4[idx].data.cpu().numpy())
            
                if e1_sp[(r_idx, e2_idx, e3_idx, e4_idx)]._nnz() > 1:
                    tmp = e1_scores[idx][e1_idx].data.cpu().numpy()
                    indic = e1_sp[(r_idx, e2_idx, e3_idx, e4_idx)]._indices()
                    e1_scores[idx][indic] = 0.0
                    if self.GPU:
                        e1_scores[idx][e1_idx] = torch.from_numpy(tmp).cuda()
                    else:
                ...
                if e2_sp[(r_idx, e1_idx, e3_idx, e4_idx)]._nnz() > 1:
                    tmp = e2_scores[idx][e2_idx].data.cpu().numpy()
                    indic = e2_sp[(r_idx, e1_idx, e3_idx,e4_idx)]._indices()
                    e2_scores[idx][indic] = 0.0
                    if self.GPU:
                        e2_scores[idx][e2_idx] = torch.from_numpy(tmp).cuda()
                    else:
                ...
                if e3_sp[(r_idx, e1_idx, e2_idx, e4_idx)]._nnz() > 1:
                    tmp = e3_scores[idx][e3_idx].data.cpu().numpy()
                    indic = e3_sp[(r_idx, e1_idx, e2_idx, e4_idx)]._indices()
                    e3_scores[idx][indic] = 0.0
                    if self.GPU:
                        e3_scores[idx][e3_idx] = torch.from_numpy(tmp).cuda()
                    else:
                ...
                if e4_sp[(r_idx, e1_idx, e2_idx, e3_idx)]._nnz() > 1:
                    tmp = e4_scores[idx][e4_idx].data.cpu().numpy()
                    indic = e4_sp[(r_idx, e1_idx, e2_idx, e3_idx)]._indices()
                    e4_scores[idx][indic] = 0.0
                    if self.GPU:
                        e4_scores[idx][e4_idx] = torch.from_numpy(tmp).cuda()
                    else:
                        e4_scores[idx][e4_idx] = torch.from_numpy(tmp)
                ...
                mrr, mr, hit = mrr_mr_hitk(e4_scores[idx], e4_idx)
                mrr_tot += mrr
                mr_tot += mr
                hit_tot += hit
